...
export default function LoginPage() {
  ...
  async function handleForgotPassword(e: React.FormEvent) {
    ...
    if (error) setMessage(error.message);
    else setMessage("重設連結已寄出，請查看你的信箱！");
    setLoading(false);
  }
  ...
  const title = isForgotPassword ? "忘記密碼" : isSignUp ? "建立帳號" : "歡迎回來";
  const subtitle = isForgotPassword ? "輸入你的 Email，我們將寄送重設連結" : isSignUp ? "加入台灣最大卡牌收藏社群" : "登入你的 Cardreasch 帳號";
          ...
          {isForgotPassword ? (
            <form onSubmit={handleForgotPassword} className="space-y-3">
              ...
              <p className="text-center text-sm text-gray-500">
                <button type="button" onClick={() => { setIsForgotPassword(false); setMessage(""); }}
              ...
              <form onSubmit={handleEmailAuth} className="space-y-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {!isSignUp && (
                      <button type="button" onClick={() => { setIsForgotPassword(true); setMessage(""); }}
